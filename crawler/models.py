...
class STOCK_DATA(TypedDict):
    '股票代碼', '總委買筆數', '總委賣筆數', '相關股票', '前一日收盤價', '跌停價', '成交流水(千股)', '委買價', '委賣價', '暫無用途', '日期（西元年月日）', '資料鍵值（交易所_代碼_日期）', '五檔賣出價格（_分隔）', '五檔買入價格（_分隔）', '股票代碼（純數字）', '時間（最後更新時間）', '股票代碼（完整）', '時間戳（毫秒）', '最終撮合時間', '五檔買入量（_分隔）', '五檔賣出量（_分隔）', '總委買張數', '今日最高價', '最小跳動單位', '單位張數', '成交價（最後成交）', '今日最低價', '股票簡稱', '開盤價', '交易所', '成交筆數', '最後成交時間', '漲停價', '成交張數', '股票全名', '昨日收盤價', '目前成交價', '每日交易資料'
